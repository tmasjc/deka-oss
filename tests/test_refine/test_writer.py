...
def _meta() -> RubricMetadata:
    return RubricMetadata(
        ...
        not_fit_examples=[
            RubricNotFitExample(pk=2, span_text="bye", fails=["x"])
        ],
...
def _cfg() -> RefineConfig:
    return RefineConfig(
        ...
        derive_model="dm", derive_base_url="x", derive_temperature=0.2,
        judge_model="jm", judge_base_url="x",
        judge_concurrency=4, judge_qps_limit=1, judge_tpm_limit=1,
        judge_timeout_seconds=30, judge_max_retries=1,
...
def _verdicts() -> list[JudgeVerdictRecord]:
    return [
        JudgeVerdictRecord(
            pk=10, nearest_fit_distance=0.1, decile=0,
            chunk_content="line a\nline b", verdict="KEEP",
            evidence_line_indices=[1, 2], failed_check=None,
            reason="good", latency_ms=42.0, attempts=1,
            rubric_version=2, prompt_sha256="b"*64,
        ),
        JudgeVerdictRecord(
            pk=20, nearest_fit_distance=0.2, decile=1,
            chunk_content="line c", verdict="DROP",
            evidence_line_indices=[1], failed_check="x",
            reason="bad", latency_ms=35.0, attempts=1,
            rubric_version=2, prompt_sha256="b"*64,
        ),
        JudgeVerdictRecord(
            pk=30, nearest_fit_distance=0.3, decile=1,
            chunk_content="line d", verdict="DROP",
            evidence_line_indices=None,
            failed_check="auto_drop_known_intruder",
            reason="auto_drop_known_intruder", latency_ms=None,
            attempts=0, rubric_version=2, prompt_sha256="b"*64,
        ),
    ]
...
def test_write_produces_four_sidecars(tmp_path):
    ...
    sample = StratifiedSample(
        ...
        auto_drop=[SampledRecord(Phase2Record(pk=30, nearest_fit_distance=0.3, raw={}), 1)],
    ...
    assert meta_obj["verdict_counts"] == {
        "KEEP": 1, "DROP": 1, "ERROR": 0, "auto_drop": 1
    }
...
def test_details_append(tmp_path):
    ...
    judge_result = JudgeResult(verdicts=[], parse_error_count=0, api_error_count=0, total_latency_ms=0.0)
...
def _stratified_sample() -> StratifiedSample:
    return StratifiedSample(
        ...
        auto_drop=[
            SampledRecord(
                Phase2Record(pk=30, nearest_fit_distance=0.3, raw={}), 1
            )
        ],
        decile_boundaries=[0.1, 0.2, 0.3],
        per_decile_count=[1, 1],
...
def test_stage_a_then_b_equals_legacy_atomic_layout(tmp_path):
    ...
    for ext in ("phase3.rubric.json", "phase3.evidence.jsonl", "phase3.prompt.md"):
        assert (
            (legacy_dir / f"abcd.{ext}").read_text()
            == (split_dir / f"abcd.{ext}").read_text()
        )

    # meta.json's `ts` field will differ (different write timestamps);
    # everything else must match.
    legacy_meta = json.loads(
        (legacy_dir / "abcd.phase3.meta.json").read_text()
    )
    split_meta = json.loads(
        (split_dir / "abcd.phase3.meta.json").read_text()
    )
    ...
    legacy_details = [
        json.loads(line)
        for line in (legacy_dir / "abcd.details.jsonl")
        .read_text()
        .strip()
        .splitlines()
    ]
    split_details = [
        json.loads(line)
        for line in (split_dir / "abcd.details.jsonl")
        .read_text()
        .strip()
        .splitlines()
    ]
